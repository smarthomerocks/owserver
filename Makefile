...
DOCKER_IMAGE_TAGNAME_ARMHF=${DOCKER_IMAGE_NAME}-armhf:${DOCKER_IMAGE_VERSION}
DOCKER_IMAGE_TAGNAME_AMD64=${DOCKER_IMAGE_NAME}-amd64:${DOCKER_IMAGE_VERSION}
...
build:
	...
	docker tag ${DOCKER_IMAGE_TAGNAME_ARMHF} ${DOCKER_IMAGE_NAME}-armhf:latest
	@echo "building image: ${DOCKER_IMAGE_TAGNAME_AMD64}"
	docker build -t ${DOCKER_IMAGE_TAGNAME_AMD64} -f amd64/Dockerfile
	docker tag ${DOCKER_IMAGE_TAGNAME_AMD64} ${DOCKER_IMAGE_NAME}-amd64:latest

push:
	docker push ${DOCKER_IMAGE_TAGNAME_ARMHF}
	docker push ${DOCKER_IMAGE_NAME}-armhf
